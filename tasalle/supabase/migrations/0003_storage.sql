...
-- migrations reste rejouable partout.
--
...
begin
    if to_regclass('storage.buckets') is null then
        raise notice 'Schéma storage absent — buckets et règles d''accès ignorés.';
        return;
    end if;

    execute $migration$    insert into storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
    values
