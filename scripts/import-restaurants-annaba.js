#!/usr/bin/env node
// Import des restaurants d'Annaba (recherche web -- TripAdvisor + sites d'annuaire locaux,
...
//   - `quartier` vient directement du JSON source quand trouve, sinon vide (plusieurs
//     adresses n'ont qu'un nom de rue, sans quartier identifiable avec confiance).
//   - Restaurants exclus de la recherche (adresse ambigue/plusieurs succursales, meme
//     critere que les imports precedents) : "La Renaissance" (deux adresses differentes
//     trouvees, 13 boulevard Chapuis ET Rte du Cap de Garde, aucune confirmee comme la
//     bonne), "Restaurant Tabarka" (deux emplacements distincts, Bd Bouzerad Hocine AAE
//     et Cours de la Revolution "Tabarka 2", ambigu lequel est reference sur TripAdvisor).
//
// Usage : SUPABASE_SERVICE_ROLE_KEY=xxx node scripts/import-restaurants-annaba.js
...
if (!SERVICE_ROLE_KEY) {
  ...
  console.error('Usage : SUPABASE_SERVICE_ROLE_KEY=xxx node scripts/import-restaurants-annaba.js');
  process.exit(1);
}
...
const DATA_PATH = path.join(__dirname, '../data/top_restaurants_annaba.json');
const UNCLAIMED_OWNER_ID = '00000000-0000-0000-0000-000000000099';

// -- cuisine_type : valeurs enum reelles en base (memes que les imports precedents) --
