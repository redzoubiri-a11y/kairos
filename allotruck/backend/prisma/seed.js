...
async function main() {
  ...
  const admin = await prisma.user.create({
    data: {
      email: 'admin@allotruck.dz',
      passwordHash,
      fullName: 'Admin AlloTruck',
      phone: '+213770000000',
      role: 'ADMIN',
    },
  });
  ...
  const clients = await Promise.all(
    [
      { email: 'client@allotruck.dz', fullName: 'Karim Belkacem', phone: '+213661111111' },
      { email: 'sarah@allotruck.dz', fullName: 'Sarah Hamdi', phone: '+213661111112' },
    ].map((data) => prisma.user.create({ data: { ...data, passwordHash, role: 'CLIENT' } }))
  );

  const transporterSpecs = [
    {
      email: 'transporteur@allotruck.dz',
