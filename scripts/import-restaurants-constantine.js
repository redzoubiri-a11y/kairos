#!/usr/bin/env node
// Import des restaurants de Constantine (recherche web -- TripAdvisor + sites d'annuaire
// locaux, PAS d'API Google Places disponible pour cette session) en fiches "draft" avec
// lien de revendication. Meme structure que scripts/import-restaurants-oran.js.
//
//   - Pas de google_place_id reel -> toujours null, idempotence par slug genere.
//   - `quartier` vient directement du JSON source quand trouve pendant la recherche,
//     laisse vide sinon (pas de lookup par rank).
//   - Restaurants exclus de la recherche (memes criteres que Alger/Oran -- adresse non
//     localisable a un quartier precis, nom source pas fiable) :
//     "Mhour Douan Mstchfa Almsalk Alboulia Ou Alkla" (nom source illisible/pas verifiable,
//     adresse en Plus Code Google uniquement -- pas de rue) ; "Pâtisserie les Falaises"
//     (Plus Code uniquement, et pâtisserie plutot que restaurant) ; "Sidi Mbrouk" (Plus
//     Code uniquement, nom generique identique au quartier -- probable doublon/entree mal
//     identifiee) ; "Laymoon Coffee Shop" (cafe, pas de numero de rue verifiable) ;
//     "Siniet El Bey" (Plus Code uniquement, pas de rue).
//
// Usage : SUPABASE_SERVICE_ROLE_KEY=xxx node scripts/import-restaurants-constantine.js
...
if (!SERVICE_ROLE_KEY) {
  ...
  console.error('Usage : SUPABASE_SERVICE_ROLE_KEY=xxx node scripts/import-restaurants-constantine.js');
  process.exit(1);
}
...
const DATA_PATH = path.join(__dirname, '../data/top_restaurants_constantine.json');
const UNCLAIMED_OWNER_ID = '00000000-0000-0000-0000-000000000099';

// -- cuisine_type : valeurs enum reelles en base (memes que les imports Alger/Oran) --
