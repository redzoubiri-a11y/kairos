...
describe('LoginPage', () => {
  it('ouvre la session pour un administrateur', async () => {
    login.mockResolvedValue({
      token: 'jeton-admin',
      user: { id: 'u1', fullName: 'Admin AlloTruck', role: 'ADMIN' },
    });

    renderLogin();
    await submit('admin@allotruck.dz', 'Password123!');
  ...
  it("refuse un compte non administrateur et ne stocke aucun jeton", async () => {
    ...
    renderLogin();
    await submit('client@allotruck.dz', 'Password123!');

    expect(await screen.findByText(/reservee aux administrateurs|réservée aux administrateurs/i)).toBeInTheDocument();
    expect(useAuthStore.getState().token).toBeNull();
    expect(localStorage.getItem('allotruck-admin-auth')).not.toContain('jeton-client');
  });
  ...
  it('refuse egalement un transporteur', async () => {
    ...
    renderLogin();
    await submit('transporteur@allotruck.dz', 'Password123!');
  ...
  it("affiche le message d'erreur renvoye par l'API", async () => {
    ...
    renderLogin();
    await submit('admin@allotruck.dz', 'mauvais');
  ...
  it('desactive la soumission tant que les deux champs ne sont pas remplis', async () => {
    ...
    await user.type(screen.getByLabelText(/adresse e-mail/i), 'admin@allotruck.dz');
    expect(button).toBeDisabled();
