#!/usr/bin/env node
// Import des restaurants d'Béjaïa (recherche web -- TripAdvisor + sites d'annuaire locaux,
...
//   - Restaurants exclus de la recherche (adresse ambigue/commune differente/pas d'adresse
//     fiable, meme critere que les imports precedents) : "Restaurant les Deux Freres" et
//     "L'Etoile de Mer" (en realite a Tichy, commune distincte de Bejaia malgre leur
//     classement dans "Bejaia Province"), "Auberge Mimosa" (en realite a Ouzellaguen,
//     ~50km de Bejaia), "Chez les Berberes" (localisation ambigue, proche d'Akbou/Ifri),
//     "Restaurant Mouradi et fils" (adresse source illisible/incoherente), "Perfect Grill"
//     (adresse = seulement route nationale N24, pas de rue/quartier precis), "Restaurant La
//     Citadelle", "Restaurant Dar Leila", "HomeBurger Bejaia", "Le Palais de la Sardine"
//     (aucune adresse ou seulement Plus Code Google sans rue nommee trouvee).
//
// Usage : SUPABASE_SERVICE_ROLE_KEY=xxx node scripts/import-restaurants-bejaia.js
...
if (!SERVICE_ROLE_KEY) {
  ...
  console.error('Usage : SUPABASE_SERVICE_ROLE_KEY=xxx node scripts/import-restaurants-bejaia.js');
  process.exit(1);
}
...
const DATA_PATH = path.join(__dirname, '../data/top_restaurants_bejaia.json');
