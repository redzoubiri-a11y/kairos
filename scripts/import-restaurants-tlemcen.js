#!/usr/bin/env node
// Import des restaurants de Tlemcen (recherche web -- TripAdvisor + sites d'annuaire locaux,
...
//   - Restaurants exclus de la recherche (adresse trop vague, meme critere que les imports
//     precedents) : "L'échappatoire" (adresse = "Tlemcen" seul, aucune rue/quartier) et
//     "Le Gourmet" (adresse = Plus Code Google seul, sans rue nommee). "Flamingo Restaurant"
//     egalement exclu : populaire mais aucune adresse fiable trouvee malgre plusieurs
//     recherches (Facebook/TikTok mentionnent le nom sans coordonnees verifiables).
//
// Usage : SUPABASE_SERVICE_ROLE_KEY=xxx node scripts/import-restaurants-tlemcen.js
...
if (!SERVICE_ROLE_KEY) {
  ...
  console.error('Usage : SUPABASE_SERVICE_ROLE_KEY=xxx node scripts/import-restaurants-tlemcen.js');
  process.exit(1);
}
...
const DATA_PATH = path.join(__dirname, '../data/top_restaurants_tlemcen.json');
