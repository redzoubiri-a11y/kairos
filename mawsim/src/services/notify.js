...
export const SMS_TEMPLATES = {
  reservation_confirmed: ({ salle, date, ref }) =>
    clampSms(`🎉 Mawsim : Votre réservation à ${salle} le ${date} est confirmée ! N° ${ref}. À très bientôt !`),
  deposit_requested: ({ pro, amount, ccp, ref }) =>
    clampSms(`💰 Mawsim : ${pro} demande un acompte de ${amount} DA. CCP : ${ccp}. Réf : ${ref}. Merci !`),
  reminder_24h: ({ type, salle, heure }) =>
    clampSms(`📅 Mawsim : Rappel — Votre ${type} à ${salle} est demain à ${heure}. Bonne fête !`),
  reservation_new: ({ client, date }) =>
    clampSms(`📨 Mawsim : Nouvelle demande de ${client} pour le ${date}. Ouvrez l'app pour répondre.`),
  review_request: ({ type, salle, lien }) =>
    clampSms(`🎉 Mawsim : Votre ${type} s'est bien passé ? Partagez votre avis sur ${salle} : ${lien}`),
  reservation_cancelled: ({ salle, date, ref }) =>
    clampSms(`❌ Mawsim : Votre demande pour ${salle} le ${date} n'a pas pu être retenue. Réf : ${ref}.`),
};
