#!/usr/bin/env node
// Import des restaurants de Blida (recherche web -- TripAdvisor + sites d'annuaire locaux,
...
//   - Restaurants exclus de la recherche (adresse non fiable/introuvable, ou hors-type) :
//     "Adlen 3 Fromage" (adresse = Plus Code Google sans rue nommee), "Cuicha" (seule une
//     adresse a Alger trouvee, pas de Blida confirmee), "Te Gusto" ("Boulevard" sans numero
//     ni nom de rue), "Atlas Memoria" (deux adresses contradictoires : Chrea vs Place de la
//     liberte n7, non resolu), "Burger Royale"/"Pourquoi Pas"/"Mtam Dar Alsltan" (aucune
//     adresse Blida trouvee), "Chicha Diplomate" (salon de chicha, pas un restaurant),
//     "Frenchy Blida" (patisserie/glacier, pas un restaurant).
//
// Usage : SUPABASE_SERVICE_ROLE_KEY=xxx node scripts/import-restaurants-blida.js
...
if (!SERVICE_ROLE_KEY) {
  ...
  console.error('Usage : SUPABASE_SERVICE_ROLE_KEY=xxx node scripts/import-restaurants-blida.js');
  process.exit(1);
}
...
const DATA_PATH = path.join(__dirname, '../data/top_restaurants_blida.json');
