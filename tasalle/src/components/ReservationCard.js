...
export default function ReservationCard({ reservation, onPress, perspective = 'client', children }) {
  ...
  return (
    // `padded={false}` : la marge intérieure et le retour d'appui restent sur
    // le Pressable, pour garder l'atténuation à 0,92 plutôt que l'ombre que
    // `MCard onPress` appliquerait. Le contour, lui, vient de MCard — c'est
    // elle qui porte la règle du §7.2, à un seul endroit.
    <MCard padded={false}>
      <Pressable
        onPress={onPress}
